...
// without a config get no opener. Plays once per session, skips on any input,
// respects prefers-reduced-motion, and runs OVER the stage while the 3D model
// loads so it costs the visitor nothing.
(function () {
  ...
  var SEEN_KEY = "ztark_intro_" + brandId;
  var debugT = qs.get("introDebug"); // pause all opener animations at t ms (dev)
  try {
    if (!debugT && sessionStorage.getItem(SEEN_KEY)) return;
    sessionStorage.setItem(SEEN_KEY, "1");
  } catch (e) { /* storage blocked: still play once */ }
  ...
  css.textContent =
    ...
    (reduced ? "" : "animation:biRise .8s cubic-bezier(.2,.7,.2,1) both") + "}" +
    ...
    (reduced ? "" : "clip-path:inset(0 100% 0 0);animation:biWipe .7s cubic-bezier(.65,0,.35,1) .15s forwards") + "}" +
    ...
    (reduced ? "opacity:0" : "animation:biGlint .8s ease-in-out 1s forwards") + "}" +
    ...
    (reduced ? "" : "transform:scaleX(0);animation:biLine .7s cubic-bezier(.65,0,.35,1) .15s forwards") + "}" +
    ...
    (reduced ? "opacity:.85" : "animation:biSub .6s ease .65s forwards") + "}" +
    ...
    "font-family:" + (cfg.font || "sans-serif") + ";opacity:0;animation:biSkip .4s ease 1.1s forwards}" +
  ...
  if (debugT) {
    ...
    setTimeout(dismiss, reduced ? 1200 : 2100);
  }
})();
