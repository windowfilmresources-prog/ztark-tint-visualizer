...
(function () {
  ...
  css.textContent =
    ...
    (reduced ? "#brandIntro .bi-lane{opacity:1!important;animation:none!important}" : "#brandIntro .bi-lane{opacity:0;transform-box:fill-box;transform-origin:28% 82%;animation:biSwoop .68s cubic-bezier(.18,.9,.28,1.06) forwards}#brandIntro .bi-lane0{animation-delay:.62s}#brandIntro .bi-lane1{animation-delay:.76s;transform-origin:35% 85%}#brandIntro .bi-lane2{animation-delay:.9s;transform-origin:42% 88%}") +
    ...
    (reduced ? "opacity:0" : "animation:biGlint .8s ease-in-out 1.7s forwards") + "}" +
    ...
    "@keyframes biSwoop{0%{opacity:0;transform:translate(-16%,34%) rotate(-8deg) scale(.72);filter:blur(3px)}62%{opacity:1;transform:translate(1.6%,-2.5%) rotate(1.2deg) scale(1.025);filter:blur(0)}100%{opacity:1;transform:none;filter:blur(0)}}" +
